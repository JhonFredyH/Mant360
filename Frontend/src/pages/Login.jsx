...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ Llamada REAL al backend con Axios + cookies HttpOnly
      const response = await api.post("/auth/login", {
        email,
        password,
      });

      // ✅ Éxito: el backend ya envió la cookie, solo redirigimos
      setBtnState("success");
      setTimeout(() => {
        window.location.href = "/dashboard"; // Redirección limpia
      }, 800);
    } catch (err) {
      // ❌ Error: manejar respuesta del backend
      setBtnState("error");
      triggerShake();
      triggerGlitch();
      setPassword("");
      ...
      if (newAttempts >= MAX_ATTEMPTS) {
        ...
        setError(
          remaining === 1
            ? `${errorMsg} — Último intento antes del bloqueo.`
            : `${errorMsg}. ${remaining} intentos restantes.`,
        );
      }
  ...
  return (
    <>
      <section className="login">
        <div
          className={`card${shaking ? " shake" : ""}${threatLevel > 0 ? ` threat-${threatLevel}` : ""}`}
        >
          <div className="header">
            ...
            <p className="subtitle">
              INDUSTRIAL MANUFACTURING PORTAL — SECURE ACCESS
            </p>
          </div>
          ...
          <div className="threat-wrap">
            <div className={`threat-label${threatLevel > 0 ? " active" : ""}`}>
              ...
              <span>
                {["NORMAL", "ELEVATED", "HIGH", "CRITICAL"][threatLevel]}
              </span>
            </div>
            <div className="threat-track">
              <div
                className="threat-fill"
                style={{ width: `${(threatLevel / MAX_ATTEMPTS) * 100}%` }}
              />
            </div>
          </div>
          ...
          <div className="panel">
            {error && !locked && (
              <div className="error-banner">
                <span className="error-icon material-icons-outlined">
                  gpp_bad
                </span>
            ...
            {locked && (
              <div className="locked-banner">
                ...
                <span className="locked-sub">
                  Se detectó actividad no autorizada.
                  <br />
                ...
                <span className="locked-note">
                  Incidente registrado en el sistema de seguridad
                </span>
              </div>
            )}
            ...
            >
              <div className="field">
                ...
                <div className={`input-wrap${error ? " has-error" : ""}`}>
                  <span className="i-left material-icons-outlined">
                    mail_lock
                  </span>
                  <input
                    id="email"
